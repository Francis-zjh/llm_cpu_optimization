...
| `data/`                          | 实验用数据集（wikitext + pg19 子集，见下方说明） |
...
**关于数据：** `data/` 中的是原始数据集的子集——wikitext 取 test 集的 5%，pg19 取 1 条记录。数据集已包含在仓库中，无需额外下载。如果本地没有数据文件，`cpu_all_optimized.py` 会自动从 HuggingFace 下载子集。
...
- Python 3.10+
- PyTorch 2.8.0（CPU 版）、IPEX 2.8.0
- Transformers、Datasets、kvpress
- psutil、tqdm
